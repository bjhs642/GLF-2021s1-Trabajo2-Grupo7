  ...
  */
  ...
  if (cambioA.length !== 0 && cambioB.length !== 0){
    if (cambioA.length !== 1 && cambioB.length !== 1){
      ...
      tabla.push({estado: estado, a: cambioA, b: cambioB});
    }

    else if (cambioA.length === 1 && cambioB.length === 1){
      console.log("PUSH... 3.3");
      tabla.push({estado: estado, a: cambioA.pop(), b: cambioB.pop()});
    }

    else if (cambioA.length === 1){
      console.log("PUSH... 3.1");
      tabla.push({estado: estado, a: cambioA.pop(), b: cambioB});
    }

    else if (cambioB.length === 1){
      console.log("PUSH... 3.2");
      tabla.push({estado: estado, a: cambioA, b: cambioB.pop()});
    }
    ...
    console.log("PUSH... 0");
    tabla.push({estado: estado, a: estado, b: estado});
    console.log("Tabla actual: ", tabla);
  }

  else if (cambioA.length === 0 && cambioB.length !== 0){//**** */
    console.log("PUSH... 1");
    tabla.push({estado: estado, a: estado, b: cambioB});
    console.log("Tabla actual: ", tabla);
  }

  else if (cambioB.length === 0 && cambioA.length !== 0){//*** */
    console.log("PUSH... 2");
    tabla.push({estado: estado, a: cambioA, b: estado});
    console.log("Tabla actual: ", tabla);
  }
}
...
//var convertir = {
  export function AFN (afnd){
  AFND= JSON.parse(JSON.stringify(afnd));
  let e, a, b;
  e = AFND[0].estado;//0 -> 0
  a = AFND[0].a;//[0,1] -> 1
  b = AFND[0].b;//2 -> null
  tabla.push({estado: e, a: a, b: b});
